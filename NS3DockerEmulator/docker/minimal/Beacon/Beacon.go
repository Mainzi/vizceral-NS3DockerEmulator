...
func beacon() {
    ...
    payload := Packet{
        Message: "Hello network! I'm " + myIP.String() + " or " + t,
        Source: myIP,
    }
    ...
    if Conn != nil {
        ...
        for {
            ...
            //log.Info(myIP.String() + " -> Send message number " + strconv.Itoa(count_msg))
            count_msg++
            CheckError(err)
            sleep_time := time.Duration(r1.Intn(20)) * time.Second
            //log.Info(myIP.String() + " -> Sleep time = " + sleep_time.String())
